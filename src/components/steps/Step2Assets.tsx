...
export const Step2Assets = ({
  ...
  const updateReasonToRemain = (id: string, reason: string) => {
    setAssets(assets.map(asset => asset.id === id ? {
      ...asset,
      reasonToRemain: reason
    } : asset));
  };
  const updateAmountToRemain = (id: string, amount: string) => {
    setAssets(assets.map(asset => asset.id === id ? {
      ...asset,
      amountToRemain: amount ? parseFloat(amount) : undefined
    } : asset));
  };
  ...
  const totalDistributableAmount = assets.reduce((sum, asset) => {
    ...
    if (asset.toRemain && asset.amountToRemain !== undefined) {
      ...
      const distributableAmount = isDebt ? asset.amountToRemain >= Math.abs(asset.amount) ? 0 : value + asset.amountToRemain : value - asset.amountToRemain;
  ...
  return <div className="max-w-4xl mx-auto">
      <Card>
        ...
        <CardContent>
          <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select className="w-full p-2 border border-border rounded-md bg-background" value={newAsset.bank} onChange={e => setNewAsset({
                ...newAsset,
                bank: e.target.value
              })}>
                    <option value="">{t('button.choose')} {t('assets.bank').toLowerCase()}</option>
                    {commonBanks.map(bank => <option key={bank} value={bank}>{bank}</option>)}
                  </select>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="accountType">{t('assets.account_type')}</Label>
                  <select className="w-full p-2 border border-border rounded-md bg-background" value={newAsset.accountType} onChange={e => setNewAsset({
                ...newAsset,
                accountType: e.target.value
                    ...
                    {newAsset.bank && getAccountTypesForBank(newAsset.bank).map(type => <option key={type} value={type}>{type}</option>)}
                  </select>
                  {newAsset.bank && <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full p-2 border border-border rounded-md bg-background" value={newAsset.assetType} onChange={e => setNewAsset({
                ...newAsset,
                assetType: e.target.value
                    ...
                    {newAsset.bank && getAssetTypesForBank(newAsset.bank).map(type => <option key={type} value={type}>{type}</option>)}
                  </select>
                  {newAsset.bank && <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="accountNumber" value={newAsset.accountNumber} onChange={e => setNewAsset({
                ...newAsset,
                ...
                <div className="space-y-2">
                  <Label htmlFor="amount">
                    {newAsset.assetType && ['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(newAsset.assetType) ? t('assets.debt') : t('assets.amount')}
                  </Label>
                  <Input id="amount" type="number" value={newAsset.amount} onChange={e => setNewAsset({
                ...newAsset,
                amount: e.target.value
              })} placeholder="0" />
                  {newAsset.assetType && ['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(newAsset.assetType) && <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-4">
                ...
                {newAsset.toRemain && <div className="space-y-4">
                    <div className="space-y-2">
                      <Label htmlFor="amountToRemain">
                        {t('assets.amount_remain')}
                      </Label>
                      <Input id="amountToRemain" type="number" value={newAsset.amountToRemain} onChange={e => setNewAsset({
                  ...newAsset,
                  amountToRemain: e.target.value
                })} placeholder="0" max={newAsset.amount} />
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="reasonToRemain">
                        {newAsset.assetType && ['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(newAsset.assetType) ? t('assets.reason_debt_remain') : t('assets.reason_remain')}
                      </Label>
                      <Textarea id="reasonToRemain" value={newAsset.reasonToRemain} onChange={e => setNewAsset({
                  ...newAsset,
                  reasonToRemain: e.target.value
                })} placeholder={newAsset.assetType && ['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(newAsset.assetType) ? t('assets.reason_placeholder_debt') : t('assets.reason_placeholder_asset')} rows={2} />
                    </div>
                  </div>}
              </div>
          ...
          {assets.length > 0 && <div className="mt-8 space-y-4">
              <h3 className="text-lg font-semibold">{t('assets.registered_assets')}</h3>
              
              {/* Group assets by bank */}
              {Object.entries(assets.reduce((groups, asset) => {
            if (!groups[asset.bank]) {
              groups[asset.bank] = [];
            }
            groups[asset.bank].push(asset);
            return groups;
          }, {} as Record<string, typeof assets>)).map(([bank, bankAssets]) => <div key={bank} className="border border-border rounded-lg p-4 space-y-4">
                  ...
                  <div className="bg-muted/50 rounded-lg p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-primary">
                        {bankAssets.filter(a => !['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(a.assetType)).reduce((sum, a) => sum + a.amount, 0).toLocaleString('sv-SE')} SEK
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                  ...
                  <div className="space-y-3">
                    ...
                    {bankAssets.map(asset => <div key={asset.id} className="border border-border/50 rounded-md p-3 space-y-3">
                        <div className="flex items-start justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Button variant="ghost" size="sm" onClick={() => toggleAssetToRemain(asset.id)} className={`p-2 ${asset.toRemain ? 'text-warning' : 'text-muted-foreground'}`} title={asset.toRemain ? t('assets.remain_tooltip_on') : t('assets.remain_tooltip_off')}>
                              {asset.toRemain ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />}
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => handleRemoveAsset(asset.id)} className="p-2 text-destructive">
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        </div>
                        
                        {asset.toRemain && <div className="space-y-3 pt-2 border-t border-border/50">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="space-y-2">
                              ...
                              <Input id={`amount-remain-${asset.id}`} type="number" value={asset.amountToRemain || ""} onChange={e => updateAmountToRemain(asset.id, e.target.value)} placeholder="0" max={asset.amount} className="text-sm" />
                              {asset.amountToRemain !== undefined && asset.amountToRemain < asset.amount && <p className="text-xs text-muted-foreground">
                            ...
                            <div className="space-y-1">
                              ...
                              <Textarea id={`reason-${asset.id}`} value={asset.reasonToRemain || ""} onChange={e => updateReasonToRemain(asset.id, e.target.value)} placeholder={t('assets.reason_remain_general')} className="text-sm" rows={2} />
                            </div>
                          </div>}
                      </div>)}
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  ...
                  <span className="text-xl font-bold text-primary">
                    {assets.filter(a => !['Bolån', 'Privatlån', 'Kreditkort', 'Blancolån', 'Billån', 'Företagslån'].includes(a.assetType)).reduce((sum, a) => sum + a.amount, 0).toLocaleString('sv-SE')} SEK
                  </span>
                </div>
                <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                  <span className="font-medium">{t('assets.total_debts')}:</span>
          ...
          <div className="flex justify-between mt-8">
            ...
            <Button onClick={onNext} disabled={assets.length === 0}>
