...
export const PhysicalAssets = ({
  ...
  const assetCategories = ["Fastighet", "Fordon", "Smycken", "Konst", "Möbler", "Elektronik", "Verktyg", "Samlingar", "Övrigt"];
  ...
  const updateDistributionMethod = (id: string, method: 'sell' | 'divide' | 'assign', assignedTo?: string) => {
    setPhysicalAssets(physicalAssets.map(asset => asset.id === id ? {
      ...asset,
      distributionMethod: method,
      assignedTo: method === 'assign' ? assignedTo : undefined
    } : asset));
  };
  ...
  const getDistributionLabel = (method: string) => {
    switch (method) {
      case 'sell':
        return 'Sälj och dela';
      case 'divide':
        return 'Dela fysiskt';
      case 'assign':
        return 'Tilldela specifik person';
      default:
        return method;
    }
  };
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {newAsset.distributionMethod === 'assign' && <div className="space-y-2">
                <Label htmlFor="assignedTo">Tilldela till</Label>
                <select className="w-full p-2 border border-border rounded-md bg-background" value={newAsset.assignedTo} onChange={e => setNewAsset({
              ...newAsset,
        ...
        {physicalAssets.length > 0 && <div className="space-y-4">
            <h4 className="font-medium">Registrerade fysiska tillgångar</h4>
            
            <div className="space-y-3">
              {physicalAssets.map(asset => <div key={asset.id} className="p-4 border border-border rounded-lg">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {asset.distributionMethod === 'assign' && asset.assignedTo && <p className="text-sm text-primary">
                  ...
                  <div className="mt-3 pt-3 border-t border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <select className="text-xs border border-border rounded px-2 py-1 bg-background" value={asset.distributionMethod} onChange={e => updateDistributionMethod(asset.id, e.target.value as any)}>
                        <option value="sell">Sälj och dela</option>
                        <option value="divide">Dela fysiskt</option>
                        <option value="assign">Tilldela person</option>
                      </select>
                      {asset.distributionMethod === 'assign' && <select className="text-xs border border-border rounded px-2 py-1 bg-background" value={asset.assignedTo || ""} onChange={e => updateDistributionMethod(asset.id, 'assign', e.target.value)}>
