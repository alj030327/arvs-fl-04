...
const TestProcess = () => {
  ...
  const [personalNumber, setPersonalNumber] = useState("195001011111");
  ...
  // Pre-fill with demo data on component mount
  useEffect(() => {
    // Pre-fill heirs with demo data
    const prefilledHeirs: Heir[] = demoConfig.mockHeirs.map(heir => ({
      personalNumber: heir.personalNumber,
      name: heir.name,
      relationship: heir.relationship,
      inheritanceShare: heir.inheritancePercentage,
      email: heir.email,
      phone: heir.phone,
      signed: false
    }));
    setHeirs(prefilledHeirs);

    // Pre-fill assets with demo data
    const prefilledAssets: Asset[] = demoConfig.mockAccounts.map((account, index) => ({
      id: `asset-${index + 1}`,
      bank: account.bank,
    ...
    setAssets(prefilledAssets);

    // Pre-fill physical assets
    const prefilledPhysicalAssets: PhysicalAsset[] = demoConfig.mockPhysicalAssets.map((asset, index) => ({
      id: `physical-${index + 1}`,
    ...
    setPhysicalAssets(prefilledPhysicalAssets);

    // Pre-fill beneficiaries based on heirs
    const prefilledBeneficiaries: Beneficiary[] = prefilledHeirs.map((heir, index) => ({
      id: `beneficiary-${index + 1}`,
      name: heir.name,
      personalNumber: heir.personalNumber,
      relationship: heir.relationship,
      percentage: heir.inheritanceShare || 50,
      accountNumber: `1234-${index + 1},567 890 ${index + 1}23-${index + 1}`,
      signed: false
    }));
    setBeneficiaries(prefilledBeneficiaries);
  }, []);
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center justify-center gap-2 text-amber-700">
            ...
            <span className="font-semibold text-sm">
              TESTVERSION - Alla fält är förifyllda med exempeldata för demonstration
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <Card className="bg-blue-50 border-blue-200">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg text-blue-900">Testversion av Komplett-paketet</CardTitle>
            <CardDescription className="text-blue-700">
              Detta är en demonstration som visar exakt samma process som den riktiga tjänsten. 
              Alla fält är förifyllda med exempeldata så att du kan se hela flödet utan att behöva fylla i uppgifter.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <span className="text-blue-800 font-medium">
                För att använda den riktiga tjänsten, gå tillbaka och välj antingen Bas- eller Komplett-paketet.
              </span>
              <Link to="/payment">
